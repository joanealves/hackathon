...
function ListaCompleta() {
  ...
  const formattedListProducts = () => {
    const newList = listProducts?.map((item) => {
      ...
      let firstDayOfWeek = moment(data_criacao)
        .weekday(Number(0))
        .format("YYYY-MM-DD ");
      let moment_last_day_of_Week = moment(data_criacao)
        .weekday(Number(6))
        .format("YYYY-MM-DD ");
      var splited_last_day_of_Week = moment_last_day_of_Week.split("-");
      var last_day_of_Week = new Date(
        splited_last_day_of_Week[0],
        splited_last_day_of_Week[1] - 1,
        splited_last_day_of_Week[2]
      );

      // primeira recorrência - de acordo com a repeticao Dia
      var data_dia_semana = moment(firstDayOfWeek)
        .weekday(Number(item?.fields?.repeticao_dia))
        .format("YYYY-MM-DD");
      var splited_data_dia_semana = data_dia_semana.split("-");
      var primeiraRecorrencia = new Date(
        splited_data_dia_semana[0],
        splited_data_dia_semana[1] - 1,
        splited_data_dia_semana[2]
      );

      let somaDias = 7 + primeiraRecorrencia.getDay();
      let isDate = new Date(
        "2022",
        Number(splited_data_dia_semana[1] - 1),
        somaDias
      );
      ...
      if (data_criacao.getDay() - item?.fields?.repeticao_dia < 0) {
        ...
        newLista = [
          ...newLista,
          { title: item?.fields?.nome, start: data, end: data },
        ];
      }
    });
    return newList;
